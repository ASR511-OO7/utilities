...
class NmapScanner:
    ...
    def detailed_scan(self, ip, ports):
        """Perform detailed scan with service detection and vulnerability scanning"""
        try:
            # Build port list
            port_list = ','.join(map(str, sorted(ports)))
            
            # Enhanced nmap command for detailed scanning
            # -sV: Version detection
            # -sC: Default scripts
            # -sT: TCP connect scan (more reliable)
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=1800  # 30 minute timeout for detailed scan
            )
            ...
            return f"Detailed scan timed out for {ip}"
        except Exception as e:
            return f"Error during detailed scan of {ip}: {e}"
    ...
    def save_text_report(self):
        ...
        filename = f"{self.output_prefix}_ports.txt"
    ...
    def generate_html_report(self):
        ...
        filename = f"{self.output_prefix}_report.html"
